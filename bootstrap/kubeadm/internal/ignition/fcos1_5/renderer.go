...
func butaneToIgnition(input []byte) (types.Config, report.Report, error) {
	// Convert through Ignition JSON to avoid duplicating Butane translation logic.
...
// Render renders the provided user data and Butane snippets into Ignition config.
func Render(input *cloudinit.BaseUserData, clc *bootstrapv1.ContainerLinuxConfig, kubeadmConfig string) ([]byte, string, error) {
	...
	if clc.AdditionalConfig != "" {
		...
		ignConfig = v3_4.Merge(ignConfig, addlIgnConfig)
	...
	return ignBytes, reports.String(), nil
}
